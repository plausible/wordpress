...
class Proxy {
	...
	public function validate_proxy_request( $request ) {
		...
		if ( $max_request_bytes > 0 && strlen( $raw_body ) > $max_request_bytes ) {
			return new WP_Error(
				'plausible_proxy_body_too_large',
				__( 'Proxy request body too large.', 'plausible-analytics' ),
				[ 'status' => 413 ]
			);
		}
		...
		if ( ! $this->has_json_content_type() ) {
			return new WP_Error(
				'plausible_proxy_invalid_content_type',
				__( 'Proxy request must be sent as JSON.', 'plausible-analytics' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! is_array( $params ) ) {
			return new WP_Error(
				'plausible_proxy_invalid_json',
				__( 'Proxy request payload must be valid JSON.', 'plausible-analytics' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! $this->has_valid_payload( $params ) ) {
			return new WP_Error(
				'plausible_proxy_invalid_payload',
				__( 'Proxy request payload is invalid.', 'plausible-analytics' ),
				[ 'status' => 400 ]
			);
		}
